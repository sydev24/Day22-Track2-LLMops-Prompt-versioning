...
def setup_vectorstore():
    ...
    chunks = split_text(text, chunk_size=900, chunk_overlap=50)
...
def main():
    ...
    subset_questions = SAMPLE_QUESTIONS[:50]
    for i, question in enumerate(subset_questions, 1):
        answer = ask(chain, question)
        print(f"[{i:02d}/{len(subset_questions)}] Q: {question[:60]}")
        print(f"       A: {str(answer)[:100]}\n")
        time.sleep(0.5)

    print(f"\n✅ {len(subset_questions)} traces đã gửi lên LangSmith project '{config.LANGSMITH_PROJECT}'")
